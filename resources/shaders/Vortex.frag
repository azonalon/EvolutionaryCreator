...
in vec2 rotation;
in float size, linewidth, antialias;
in vec4 fg_color, bg_color;
in float v_size;
...
float arrow_stealth(vec2 texcoord, float body, float head, float linewidth,
                    float antialias) {
...
void main() {
  ...
  float distance = arrow_stealth(P * v_size, size, 0.5*size, linewidth, antialias);
  FragColor = outline(distance, linewidth, antialias, fg_color, bg_color);
//   FragColor = filled(distance, linewidth, antialias, bg_color);
}