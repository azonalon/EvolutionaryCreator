...
ElasticModel *rectangularModel(OpenGLContext &context)
{
    ...
    loadMesh2D("resources/meshes/triangle.obj", vertices, indices, 0, 0);
    const std::vector<double> k(indices.size(), mu);
    const std::vector<double> nu(indices.size(), lambda);
    const std::vector<double> M(vertices.size(), 2);
    ...
    // em->insertParticle(px, py, vpx, vpy, Mpx, Mpy);
    em->insertParticle(0.75, 0.25, 0, 0, 1, 1);
    ...
    em->computeStaticPotentialGradient = [&](const ElasticModel *m, const auto &x, auto &g) {
        ...
        if(pos.x < 1e30 && pos.x > -1e30) {
            for (unsigned i = 0; i < x.size() / 2; i++)
            {
                auto v = glm::vec2(x[2*i], x[2*i+1]) - pos;
                E += cStatic * (v.x) * (v.x) * m->M[2 * i];
                g[2 * i    ] += 2 * cStatic * (v.x) * m->M[2 * i    ];
                g[2 * i + 1] += 2 * cStatic * (v.y) * m->M[2 * i + 1];
            };
            return E;
        }
        return 0.0;
    ...
    em->computeStaticPotentialDifferential = [&](const ElasticModel *m, const auto &x,
        ...
        if(pos.x < 1e30 && pos.x > -1e30) {
            ...
            {
                auto v = glm::vec2(x[2*i], x[2*i+1]) - context.cursorPosWorld();
                dg[2*i    ] += 2 * cStatic * m->M[2*i]     * dx[2*i    ];
                dg[2*i + 1] += 2 * cStatic * m->M[2*i + 1] * dx[2*i + 1];
            };
        }
    };
